...
module.exports = {
    customId: 'dash_select_endQuest_', // Prefix match
    async handle (interaction) {
        try {
            ...
            if (quest.isArchived) {
                return interaction.editReply({ content: '⚠️ このクエストは既に終了済みです。' });
            }
            ...
            await interaction.editReply({ content: `✅ クエスト「${quest.name}」を終了状態にしました。` });

        } catch (error) {
            await handleInteractionError({ interaction, error, context: 'ダッシュボードからのクエスト終了' });
        }
    },
};