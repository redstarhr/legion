  ...
  async handle(interaction) {
    try {
      ...
      if (hasAlreadyAccepted) {
        return interaction.reply({ content: '⚠️ あなたは既にこのクエストを受注済みです。受注内容を変更する場合は、一度「受注取消」を行ってから再度受注してください。', ephemeral: true });
      }
      ...
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: 'エラーが発生したため、受注を開始できませんでした。', ephemeral: true }).catch(console.error);
      }
    }
  },
};