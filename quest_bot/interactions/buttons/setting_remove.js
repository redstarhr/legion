const { setLogChannel, setNotificationChannel } = require('../../utils/configManager');
    ...
    async handle(interaction) {
        try {
            ...
            switch (settingToRemove) {
                case 'log_channel':
                    await setLogChannel(interaction.guildId, null);
                    logTitle = '⚙️ ログチャンネル設定解除';
                    successMessage = '✅ ログチャンネルの設定を解除しました。';
                    break;
                case 'notification_channel':
                    await setNotificationChannel(interaction.guildId, null);
